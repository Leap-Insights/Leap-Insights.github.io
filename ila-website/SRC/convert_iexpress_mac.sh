...
pushd /tmp
curl -s $INPUT_URL > article.html || exit
...
echo "== Finding title"
## TITLE=`grep -i '[<]h1' $INPUT | sed 's/<[/]\?[A-z|0-9]*[^>]*>//g'`
TITLE=`grep -i '[<]h1' $INPUT | sed -e 's/<[^>]*>//g' -e 's/^[ ]*//' | tr -d [:cntrl:]`
